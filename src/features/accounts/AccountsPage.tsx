...
import { Chip, MenuItem, Select, FormControl, InputLabel, Button, Dialog, DialogTitle, DialogContent, DialogActions } from '@mui/material';
...
export function AccountsPage() {
  ...
  const columns: AdminTableColumn<AccountAdminDto>[] = [
    ...
    { id: 'role', header: 'Vai trò', render: (r) => <Chip label={r.role} size="small" color={r.role === 'Admin' ? 'error' : 'default'} /> },
    { id: 'status', header: 'Trạng thái', render: (r) => <Chip label={r.status} size="small" color={r.status === 'Active' ? 'success' : 'warning'} /> },
  ];
    ...
    <>
      ...
      */}
      <ChangeStatusDialog 
        ...
        onSuccess={() => { setSelectedAccount(null); fetchAccounts(); }} 
      />
    </>
  );
}
...
function ChangeStatusDialog({ account, onClose, onSuccess }: { account: AccountAdminDto | null, onClose: () => void, onSuccess: () => void }) {
